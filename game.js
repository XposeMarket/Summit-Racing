...
class Game {
  ...
  setCameraImmediate(){const f=new THREE.Vector3(Math.sin(this.player.yaw),0,Math.cos(this.player.yaw)),dist=this.isMobile?5.1:7.2,height=this.isMobile?2.75:3.5,lookAhead=this.isMobile?4.0:5;this.camera.position.copy(this.player.position).addScaledVector(f,-dist).add(new THREE.Vector3(0,height,0));this.camera.lookAt(this.player.position.clone().addScaledVector(f,lookAhead).add(new THREE.Vector3(0,this.isMobile?.82:1,0)));this.camera.fov=this.isMobile?61.5:68;this.camera.updateProjectionMatrix();}
  ...
  updateCamera(dt){
    const f=this.tmpForward.set(Math.sin(this.player.yaw),0,Math.cos(this.player.yaw)),speedRatio=clamp(this.player.speed/this.player.raceTopSpeed,0,1.4),mobile=this.isMobile,dist=(mobile?lerp(4.95,5.75,clamp(speedRatio,0,1)):lerp(6.8,8.6,clamp(speedRatio,0,1)))+(this.player.boosting?(mobile?.26:1.3):0),height=mobile?lerp(2.55,2.95,clamp(speedRatio,0,1)):lerp(3.1,3.75,clamp(speedRatio,0,1));
